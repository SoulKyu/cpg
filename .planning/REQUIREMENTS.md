...
- [ ] **VIS-04**: Pre-flight check (`cpg generate --l7` only) reads ConfigMap `kube-system/cilium-config`, verifies `enable-l7-proxy: "true"`. If false or missing, emit a warning naming the offending field with a remediation hint (set in ConfigMap and roll the cilium-agent DaemonSet). On RBAC denied: warn-and-proceed.
- [ ] **VIS-05**: Pre-flight check (`cpg generate --l7` only) verifies presence of cilium-envoy: DaemonSet `kube-system/cilium-envoy` (Cilium ≥ 1.16) OR detection by the `enable-envoy-config` flag in cilium-config (Cilium 1.14–1.15 embeds envoy in cilium-agent — check passes silently). On RBAC denied: warn-and-proceed.
