...
2. **RESOLVED — Should the SEC-01 audit's fs-write allowlist match on the exact call-site (caller, callee) pair, or on the caller function alone?** → Resolution adopted: per-function keying (Plan 19-01's explicit design choice, matching D-03's phrasing).
